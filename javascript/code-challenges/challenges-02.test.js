...
const raisedToTheThird = (arr) => {
  ...
  console.log(arr);
  // The map function creates a new array containing the processed results. No need to push them to a new array
  let raisedNum = arr.map(function (num) {
...
const addOne = (arr) => {
  ...
  let added = arr.map(function (num) {
    return num + 1
  });
...
const addQuestion = (arr) => {
  ...
  let exclaim = arr.map(function (num) {
    return num + "?"
  });
  //Return the entire array
  return (exclaim);
...
const forLoopTwoToThe = (arr) => {
  // Using math fuinction...Math.pow(base, exponent)
  let raised = arr.map(function (num) {
...
const mapTwoToThe = (arr) => {
  // Solution code here...
    // Solution code here...
    let ans= arr.map((element) => Math.pow(2,element));
